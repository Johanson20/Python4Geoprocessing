...
Z = int(arcpy.GetParameterAsText(1))
AF = int(arcpy.GetParameterAsText(2))
T = int(arcpy.GetParameterAsText(3))
outFieldName = arcpy.GetParameterAsText(4)

# Add output field, if it doesn't already exist
fields = arcpy.ListFields(fc)
fieldNames = [field.name for field in fields]
if not outFieldName in fieldNames:
    arcpy.AddField_management(fc, outFieldName, 'FLOAT')
...
results = arcpy.CalculateField_management(fc, outFieldName, value)
