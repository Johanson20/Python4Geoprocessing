...
intList = [round(float(val), 0) for val in strList]   ### modify this
...
reciprocal = [1/no for no in values]  ### modify this
...
fieldNames2 = [val.replace('/', '_') for val in fieldNames]  ### modify this
print('4. No slashes:', fieldNames2)

# Create a list of output file names (with the word 'out' inserted before the extension).
inputFiles = os.listdir('C:\Users\owner\Downloads\Sample_scripts\ch04')
outputFiles = [os.path.splitext(f)[0] + 'out' + os.path.splitext(f)[1] for f in inputFiles]  ### modify this
print('5. Output files:', outputFiles)

# Create a list file extensions.
inputFiles = os.listdir('C:\Users\owner\Pictures')
extensions = [os.path.splitext(f)[1] for f in inputFiles]  ### modify this
